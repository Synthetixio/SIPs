...
- `SystemSettings.atomicEquivalentForSynth`: a synth's equivalent on-chain asset with higher on-chain liquidity to poll TWAP prices from. Having this specified for a synth will also allow it to be used as a source synth or destination synth in atomic exchanges.
...
- `P_CLBUF`: `P_CL`, the current Chainlink price, is the official “internal” price used for all other exchanges and system debt calculations. However, its update latency is easily gamed via technical frontrunning in today’s circumstances. `P_CLBUF` provides a buffer of *N* bps from `P_CL` to provide a safety net from the deviation threshold for Chainlink oracle updates.
...
- `SystemSettings.atomicEquivalentForSynth` (`mapping (bytes32 => address)`) and related setter `SystemSettings.setAtomicEquivalentForSynth()`, configurable by SCCPs
...
1. Sanity check the current exchange's Chainlink rates against the internal circuit breaker
...
For step 2, note that due to the low liquidity of synths on Uniswap and Sushiswap, queries to `Keep3rV4OracleUSD` will have the option to use an equivalent non-synth with high liquidity instead. This can be configured via SCCPs by calling `SystemSettings.setAtomicEquivalentForSynth()`.

In the event the [`Keep3rV4OracleUSD` oracle aggregation contract
(`0x6bcc7ea24d97da4f14b564511501490602d361f2`)](http://etherscan.io/address/0x6bcc7ea24d97da4f14b564511501490602d361f2#code) needs to be replaced, the owner of `ExchangeRates` will have the ability to update its address.
