...
Now that this limitation is about to be lifted and with the upcoming support of Chainlink oracles, all the necessary conditions are met for enabling synth exchanging and make the transition to Phase 1.
...
No new contracts need to be developed. The OE Synthetix instance, namely `MintableSynthetix` should be enhanced with exchanging functionality. The basic Synthetix functionality that is suported on both layers is implemented on a parent contract (`BaseSynthetix`) which is inherited by the L1 (Synthetix) and the l2 (MintableSynthetix) instances.
...
The polymorphic approach adopted during the L2 integration (check [SIP-102](./sip-102.md) for further details) is very efficient: for basic exchanging functionality, the only functions that need to be enabled are the following: `exchange()`, `exchangeOnBehalf()`,`settle()`. Their implementation has to be moved from `Synthetix` to `BaseSynthetix` since they are going to be available on both layers.
