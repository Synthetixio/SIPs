...
In terms of the next bridge upgrade, the idea would be that after the new contracts are deployed, the system is suspended, and Optimism finalizes all withdrawals, the old `SynthetixBridgeToOptimism`'s migrate function will be called, effectively disabling it, and transferring all funds to `OptimismDeposit`.
