...
\\(feepool reward \ := p_a f_r \\)

and the keeper reward (which is the msg.sender of the liquidation transaction) would be:

\\(keeper reward \ := p_a k_r \\)

In the scenario where the borrower liquidates their own locked collateral to repay the borrowed synths, they would recieve 75% of the penalty amount.
#### Interaction Circuit Breakers on Shorts:
