...
sETH is freshly minted whenever a user deposits ETH into the contract. The user can deposit any amount desired however, this is subject to not exceeding the `maxETH` configurable via SCCP. 
There is no duration, interest rate or collateralization ratio, as any user can at any time buy back the WETH deposited in the contract by burning sETH.
...
The contract supports both ETH and WETH for deposits, and automatically handles wrapping ETH for the user.
