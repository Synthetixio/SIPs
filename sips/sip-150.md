...
immediately after issuing synths, the issuer's debt balance is less than the sUSD they actually mint.
...
will be underreported until a snapshot.

### Rationale

The existing logic is incorrect and needs to be corrected, which this SIP proposes to do. It should be noted that
as it stands, the offending code will now be completely unused. This is intentional until a more robust pass on the
debt pool with the upcoming L1/L2 debt pool unification.

### Technical Specification

No changes will be made to the external API, the fix is limited to internal functions only.
...
is not to perform any logic related to `excludedDebt` at all during the issuance process, as not only
does it resolve this issue, but it is unnecessary.
