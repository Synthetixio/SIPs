...
- `SystemSettings.synthCanTradeAtomically`: whether a synth can be used as a source synth or destination synth in an atomic exchange. Note that both the source and destination synths must be allowed for an atomic exchange between them to be allowed.
...
- `SystemSettings.atomicPriceWindow` (`TWAP_WINDOW`): the time window to use for TWAP, specified in number of 30min periods
...
- `Exchanger.lastAtomicVolume` (`struct { uint128 block, uint128 volume }`), `SystemSettings.maxAtomicVolumePerBlock` (`uint256`), and related setter `SystemSettings.setMaxAtomicVolumePerBlock()`, configurable by SCCPs
- `SystemSettings.synthCanTradeAtomically` (`mapping (address => bool)`) and related setter `SystemSettings.setSynthCanTradeAtomically()`, configurable by SCCPs
- `SystemSettings.atomicPriceBuffer` (`uint256`) and related setter `SystemSettings.setAtomicPriceBuffer()`, configurable by SCCPs
- `SystemSettings.atomicPriceWindow` (`uint256`) and related setter `SystemSettings.setAtomicPriceWindow()`, configurable by SCCPs
...
1. Execute the exchange by burning source synth, issuing destination synth, and collecting fees. Crucially, this step issues destination synths directly to the exchanger and does not create new virtual synths, bypassing fee reclamation. Also note that the fee collected will be based on the observed price from 6., which may not be the internal oracle price.
1. Remit the fee, if required
...
For step 2, note that due to the low liquidity of synths on Uniswap and Sushiswap, queries to `Keep3rV1OracleUSD` will use an equivalent non-synth with high liquidity instead:
...
- Synths allowed, specified in booleans
