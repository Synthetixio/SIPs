...
The `issuer` contract will calculate the amount of SNX that is liquidated + penalty based on the liquidation type and the sUSD debt to burn from the liquidated staker. The debt shares of the liquidated staker is updated (during the debt burn action) within the liquidation transaction before the liquidated SNX is rewarded to the liquidated SNX contract.

The effective debt % of other stakers would increase after the liquidated staker's debt is burned.

The SNX claimed will be escrowed for 12 months on the escrow contract.
