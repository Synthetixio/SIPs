...
| \\(I\\) | Interest accrued on the loan | sETH |
...
| \\(i\\) | Instantaneous interest rate | \\[i \ := \ clamp(W + b, 0, 1) \ i_{max} \\]  | A percentage per day. |
...
- `baseInterestRate` the rate that is applied to all shorts regardless of currency.
