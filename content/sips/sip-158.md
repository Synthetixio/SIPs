...
Once the synths are deprecated, the collateral manager will be updated to remove references to iSynths.
...
### Phase 1: Deprecate Inverse Synths

1. Pause exchanges on all iSynths.
2. Remove iSynths from the Synthetix (deprecating them) and inverse pricing exchangeRates contract.
...
Splitting the deprecation of iSynths into two phases allows the isynths to be deprecated and removed from Synthetix exchange first, and then have contract updates to be done as a separate release.
