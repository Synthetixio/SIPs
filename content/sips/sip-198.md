...
- `getPureChainlinkPriceForAtomicSwapsEnabled` is a internal function in `MixinSystemSettings` that returns True if an asset has been tagged to trade at purely the chainlink price and False otherwise 
...
In order to arrive to the **trade price**, quoted as `DEST/SRC`, we divide the `DEST/USD` by the `SRC/USD`. For example trading 1 bitcoin into euros, this is computed by getting the components, `EUR/USD` (1.05$ per eur) and `BTC/USD` (38k$ per btc). Then dividing `Dest/USD` by `SRC/USD` (i.e. `1.05/38k`), resulting in `0.0000276` bitcoin per euros (`BTC/EUR`).
Assuming either `SRC` or `DEST` is actually the US Dollar, the same logic still applies, where one of the components is `USD/USD` or unity. An example of this is a trade of 1 USD into BTC, in such a situation the trade price `DEST/SRC` is obtained by dividing `BTC/USD` by `USD/USD`, leading to 38k$.
...
When a chainlink price is assigned to either source ro destination, the chainlink price is used. Otherwise the **worse** price between chainlink and uniswap is used in the combination, the end goal is to use the lowest `DEST/SRC` possible. Below are the different scenarios that could arise, where we denoted `_Uni` a price sourced from the uniswap V3 based `DexPriceAggregatorUniswapV3` contract and `_Chainlink` is the price from the chainlink price feed:
  ...
  - Given that the price of `EUR` from chainlink is at `1.1` while from uniswap is at 1.2. 
      - Given that The price of `BTC` on uniswap is `37,000` while chainlink is at `38,000`.
        - Given that `sEUR`and `sBTC` are not set to trade at the pure chainlink price
          - When the user attempts to trade, setting the minReturnAmount to `300,000`
            - ✅ Then it succeeds and the following take place:
              - `*308,333` sEUR is sent to the user, and `10` `sBTC` are burned
        ...
        - Given that `sBTC` and `sEUR` are both set to trade at the pure chainlink price
          - When the user attempts to trade, setting the minReturnAmount to `300,000`
            - ✅ Then it succeeds and the following take place:
              - `*345,454` sEUR is sent to the user, and `10` `sBTC` are burned
        ...
        - Given that `sBTC` is set to trade at the pure chainlink price and `sEUR` is not set to trade at the pure chainlink price
          - When the user attempts to trade, setting the minReturnAmount to `300,000`
            - ✅ Then it succeeds and the following take place:
              - `*316,666` sEUR is sent to the user, and `10` `sBTC` are burned
        ...
        - Given that `sEUR` is set to trade at the pure chainlink price and `sBTC` is not set to trade at the pure chainlink price
          - When the user attempts to trade, setting the minReturnAmount to `330,000`
            - ✅ Then it succeeds and the following take place:
              - `*336,363.6` sEUR is sent to the user, and `10` `sBTC` are burned
