...
- The calculation steps of the minKeeperFee is as follows (detailed example below):
 1) `costOfExecutionGross := (l2GasPrice * l2GasUnits + l1GasCost) * ETH/USD`
...
- Computation of `l2GasPrice`and `l1GasCost` are detailed [here](#on-chain-gas-price-values)
...
#### L2 Gas Cost:
The L2 Gas cost remains unchanged and is calculated by fetching the l2GasPrice (`block.baseFee`) and multiplying by the configurable value `l2GasUnits`.
