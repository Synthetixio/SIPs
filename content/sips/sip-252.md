...
- A flagged account that self-liquidates sucessfully, will not have his flag removed automatically. The account will still be able to remove the flag, after burning to target if necessary (to ensure his cratio is restored to health) then using the function `checkAndRemoveAcccountInLiquidation`.
