...
- A flagged account that self-liquidates sucessfully, will not have his flag removed automatically. The account will still be able to remove the flag, after burning to target if necessary (to ensure his cratio is restored) then using the function `checkAndRemoveAcccountInLiquidation`.
...
Note that if a user doesn't not hold enough collateral to restore to health, meaning if `collateral / debt < (1+liquidationPenalty)`, then the account would not be able to call self-liquidate. The main intuition behind this requirement is to allow accounts to be removed from the system, in the event the value of their collateral is below the amount necessary to retore to health.
