...
2) Switching from the use of 30 bp uniswap pools to the 5 bp uniswap pools as default pools in `DexPriceAggregatorUniswapV3` contract.
...
Furthermore, it is not possible to route trades on the wBTC/USDC 5 bp pool as it does not exist on uniswap. Creating a new `DexPriceAggregatorUniswapV3` with default fee of 5 bp will ensure that prices are gathered from the 5 bp pools by hoping over wBTC/wETH and wETH/USDC 5 bp pools as a workaround and finding the relevant price of the sBTC <> sUSD trade. 
...
This cannot be implemented using the setPoolForRoute as done in [SCCP-210](https://sips.synthetix.io/sccp/sccp-210/) because wBTC/USDC 5 bp pools do not exist. However, wBTC/wETH and wETH/USDC 5 bp pools do exist which allow the computation the `wBTC/USDC` price by hoping over the relevant pools with the `_fetchAmountCrossingPools` method.
