...
It is worth mentioning as a consequence of this fix, all future contracts that would receive snx distributions, will need  to implement a `notifyReward` in the receiving contract, otherwise the distribution and in consequency the weekly inflation print will fail. Currently, as per the state of the v2X contracts, at the time of writing this sip, no other contracts are configured to receive snx inflation on a weekly basis, aside from L2 stakers.
