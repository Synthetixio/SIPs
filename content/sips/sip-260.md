...
* `isExceedingRateLimit`
  * verify correct behavior when 0 
...
* `SynthetixBridgeToOptimism.setRateLimitPeriod(bytes32 currencyKey)` -- number of seconds between when the amount received gets reset
* `SynthetixBridgeToOptimism.setRateLimitAmountUsable(bytes32 currencyKey)` -- amount of token which can be received or sent every period term
