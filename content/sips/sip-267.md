...
- Test on atomic exchange fees
    ...
    - Setting a Direct Integration (DI) account to have 0 bp sEUR atomic exchange fees and 0 bp sUSD fees, while standard fees being 15 bp sETH and 5 bp on sUSD
        - When the DI account attempts to trade atomically 100 sUSD to sETH
          - ✅ Then it succeeds and the following take place:
              - 15 bp fees are levied on the trade, being the default fees
    - Setting a Direct Integration (DI) `maxAtomicVolumePerBlock` to 2m $ per block while the standard parameter being 100k$ 
        - When a non DI account attempts to trade 2m sUSD to sETH
          - ❌ Then transaction reverts, due to exceeding max volume per block
        - When an DI account attempts to trade 2m sUSD to sETH and simulatenously in the same block a non-DI account trades 100k sUSD to sETH
          - ✅ Then both transactions succeed
        - When an DI account attempts to trade 2.1m sUSD to sETH and simulatenously in the same block a non-DI account trades 100k sUSD to sETH
          - ❌ Then the DI account transaction reverts
          - ✅ Then the non-DI account transaction suceeds
    - Setting a Direct Integration (DI) `atomicVolatilityUpdateThreshold` of 3 updates while the standard parameters being 5 updates.
        - When 3 oracle updates are pushed in the last minute
