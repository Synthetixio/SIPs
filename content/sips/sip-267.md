...
`setIntegration` allows the protocol to specify the parameters of the exchange functionalities that go through the `DirectIntegrationManager` contract. It takes in the following structure:
...
Note by default parameters that are not set for certain currency keys (such as setting the enviroment for `sETH` but not `sUSD`) result in the integration contract being able to interact at the default parameters pertaining to the un-configured currency keys.
...
- `setExchangeParameters`: This will allow the protocol to configure exchange functions that go through `DirectIntegration` contract, requires a SIP presentation.
