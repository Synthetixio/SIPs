...
Note by default parameters that are not set for certain currency keys (such as setting the enviroment for `sETH` but not `sUSD`) take on the default exchange parameters.
...
- `exchangeAtomically(bytes32,uint,bytes32,bytes32,uint)`: [`setDexPriceAggregator`,`setAtomicEquivalentForDexPricing`,`setAtomicExchangeFeeRate`,`setAtomicMaxVolumePerBlock`,`atomicVolatilityUpdateThreshold`,`atomicVolatilityConsiderationWindow`]
