...
This function return the set of settings configured for a given address and currencyKey combination. If an address is not configured with any overrides, this would just return the global parameters from system settings. Any overrides would be applied if applicable. 
