...
status: Draft
type: Governance
author: 'Roger Brogan (@rbrogan-git), MEB (@barrasso)'
...
Token transfers using the CCIP token pools provide Synthetix the opportunity, if desired, to open up CCIP's private snxUSD Token Pool which would allow third parties to transfer snxUSD (even in combination with other tokens and/or data).
