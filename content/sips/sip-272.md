...
### Bounds on slippage
Note that `f(x)` is bounded between 0 and 1
...
- At blockNumber 10, Swap 1m$ sUSD to sETH with atomic price at 1,600$ per ETH
  ...
  2) blockNumber in `cumulativeVolume` structure doesn't need updating (since currentBlockNumber less blockNumber in structure < 2)
  3) cumulativeVolume updated to 1m$
  4) User receives 1m$ / 1600$ * (1-13.5bp) = 624.18 ETH
...
- At blockNumber 11, Swap 624.18 sETH to sUSD with atomic price at 1,600$ per ETH
  ...
  4) f(|-1,315$|) = 0
  5) User receives 624.18 eth * 1600$  = 998,685$
...
- At blockNumber 12, Swap 625 sETH to sUSD with atomic price at 1,600$ per ETH
  ...
  3) `cumulativeVolume` is reset to zero, then cumulativeVolume is updated to -1m$
  4) f(|1m$|) = 13.15 bp
  6) User receives 625 * 1600 * (1-13.5bp) = 998,685$

- At blockNumber 13, Swap 1m$ sUSD back to sETH with atomic price at 1,600$ per ETH
  1) volumeInUSD = 1m$
  2) blockNumber in `cumulativeVolume` does not need an update
  3) `cumulativeVolume` is updated to 0$
  4) f(|0|) = 0
  6) User receives 998,685$ / 1600 = 624.18 ETH 
