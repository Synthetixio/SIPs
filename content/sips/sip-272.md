...
$$
G(x,y) =  2 * [u_0 (x-y) + \frac{2}{3} u_1 (x ^ {3/2} - y^ {3/2} ) + \frac{1}{2} u_2 (x^2 - y^2) + \frac{1}{3} u_3 (x^3-y^3)] / (x-y)
$$

- $y$ is the pre-trade cumulative volume in USD
- $x$ is the post-trade cumulative volume in USD
- both $x$ and $y$ are absolute numbers
- In case cumulative volume flips signs due to a significant trade in the opposing direction, then x is set to |x-y| **then** y is set to zero in the function above
