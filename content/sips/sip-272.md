...
However, given the necessity to ensure that trade chunks are agnostic to volume flow (i.e. how small consecutive trade transactions or a one-lump sum transaction result in the same weighted average fee), then the integral, F(x) is used. F(x) represents the area under the slippage curve and is not affected by the size of an individual trade. In order to compute the weighted average fee, `2F(x)/x` is used which gives back the weighted average slippage from executing a trade with a volume of `x` . 
