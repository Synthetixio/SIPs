...
$$
2 \frac{ F(x)} {x} = +1.009 * 10^1 -2.379 * 10^{-2} \sqrt{x} +2.297 * 10^{-5}x -1.418 * 10^{-12}x^2 
$$
...
- dynamic fees are constrained between 0 and `maxAtomicDynamicFee`, configurable via SCCP
 ...
 The exchange would take place with the following computation, assuming that `cumulativeVolume` starts at y, having the same sign as x: 

- if the sourceCurrencyKey is `sUSD`:

`SrcAmount * 1/P * (1- baseFee) * [1-G(x,y)] => DestAmount`
