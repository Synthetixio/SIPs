...
The dynamic fee will be sent to the fee pool (or burned depending on the implementation of [SIP-255](https://sips.synthetix.io/sips/sip-255/)) upon every atomic exchange. Hence, the fee pool should receive `SourceAmount*Price*(baseFee + dynamicFee)`  upon execution of a trade.
...
This SIP has been rejected by the author.
