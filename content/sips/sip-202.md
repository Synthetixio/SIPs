...
A modified supply schedule contract that takes an amount of SNX supply to mint for a week based on the calculated amount that is set by the protocolDAO. The weekly supply amount is derived from the `base amount` and the previous amount of SNX minted for staking rewards plus the increment of 10% (`positive rebase`) if the target staking ratio of 85% has been missed by more than 5% or a reduction of 10% from the last minted amount if it has been exceeded by more than 5% (`negative rebase`). If the actual staking ratio is within +/- 5% of the target of 85% a reduction of 5% from the last minted amount will be implemented (`decay rebase`) 

The amount will be calculated off-chain during phase one until the implementation of phase two which will be computed on-chain using a Chainlink oracle.
