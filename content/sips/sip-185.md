...
If this SIP is approved, the Synthetix Governance system will be expanded to include the ratification of a Risk Committee consisting of 5 members nominated from the existing governing bodies.

They will be responsible for:

- Setting a process for the analysis of risk within the Synthetix Protocol (i.e monetary policies, external partnerships/programs)
...
In a complex system such as the Synthetix Protocol, there is limited oversight and responsibility on the factors of risk that impact the Protocol. These risk can be introduced via changes internally, such as through the SIP process or externally, through ecosystem economics that may affect the Synthetix Network value.

As we continue moving towards decentralized governing bodies, it is important that there exist a body that is able to focus on the risk element of the Synthetix Protocol.
...
The Risk Committee (RC) will be in responsible for managing and responding to the risk of the Synthetix Protocol. More specifically they initially will be in charge of:
...
If a SIP is appointed this label, the RC has found that there is negligible risk in the proposed SIP and it can be then considered for voting and implementation.

_Caution_

SIPs marked with this label mean that the SIP can proceed but the Spartan Council should take this label and the accompanying summary as precautions to consider when debating and voting on the SIP.

_Review Required_

This label, blocks the SIP from moving on and the SIP author must address the concerns raised by the RC before it can progress.
...
\*Criteria - The RC is to introduce a criteria that outlines the certain risk that they propose are unacceptable to the Synthetix Protocol, since this criteria will be used to veto pending SIPs and is considered a meta-governance issue they must present the criteria and any changes

The RC will also be able to introduce a check-list in this process that when failed will automatically reject the proposed SIP due to overwhelming risk.

This check-list is considered a meta-governance issue and any changes will require unanimous vote cast by the Spartan Council.

#### Protocol Risk Oversight

Historically the management of risk has been dealt with in an ad-hoc nature by Core Contributors. The RC will be appointed the body to be responsible for managing risk in protocol policy changes (usually exposed via SCCPs - such as proposed inflation or c-ratio changes). For SCCPs, the same process outlined in the SIP workflow applies.
...
The existing system whereby risk is not appointed to a particular individual or body presents an existential risk to the Protocol. Although no major risk events have happened so far, the rapid growth and development of the Synthetix Protocol and ecosystem has increased the bandwidth required to manage risk within Synthetix.

The design of the Risk Committee was motivated by the status-quo of the current Synthetix Governance system, whereby the most critical aspects are appointed to delegated bodies.
...
The Risk Committee schedule a meeting to discuss how the introduction of liquidity incentive pools (in which sUSD is apart of the proposed plan) might affect the system and they come to conclusion that it might cause the sUSD peg to be off. They label it "Review Required" as they think it might be viable to pass if they remove the sUSD pool from the proposal. This means that the Spartan Council does not yet need to debate this issue as it is blocked until changes are implemented.
...
The proposal ultimately ends up failing to pass.

#### Internal Protocol Oversight

A SCCP is made that proposes to decrease the c-ratio to 10%, the Risk Committee upon seeing this SCCP holds a discussion and ultimately comes to the conclusion that it has failed their internal criteria and assigns it a risk summary of "Not Acceptable" the SCCP is automatically rejected.
