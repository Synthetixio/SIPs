...
- **Administration Period** - At the beginning of this period, anyone can call the `resolve` function on the mothership. NFTs are distributed to the nominees which have received the most votes in the previous cycle. *Wormhole Queries* is used to gather all of the votes cast across any satellite deployments. A cross-chain message is sent via *Wormhole Messages* such that the NFTs controlled by the satellite deployments are redistributed as well. Throughout the administration period, addresses can be nominated for the following election period.
...
This is system is fully permissionless. There is no need for specific addresses to have elevated permissions, the creation of merkle trees, or reliance on keepers. Election winners will be motivated to call the `resolve` function as soon as possible, such that they receive NFTs. The`commence` function can be triggered automatically when the first vote is cast in the election period, if necessary.
