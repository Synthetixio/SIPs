...
Aelin is a new protocol and will have its own token, community, and governance system. Initially, Aelin v1 will be governed by the Aelin Council, a DAO which will be voted on by SNX stakers. The current weighted debt of SNX stakers will be used to vote on 5 Aelin Council members.
...
For simplicity, the initial format of Aelin Improvement Proposals (AEIP) will follow the Synthetix Improvement Proposals (SIP) standard, which has been refined by Synthetix over time. Anyone from the Aelin community may submit AEIPs and be rewarded in an amount decided by the Aelin DAO.
...
Aelin holders can stake their tokens to capture deal fees and inflationary rewards. The spec for the staking rewards contract is TBD. Aelin v1 will capture deal fees in a multisig with distribution later. The inflationary rewards will start when the staking rewards contract is launched.
