...
- Given there was previously an exchange for `sUSD` (or any synth) to `sETH`, recording the last sETH price as `200`
  ...
  - And the current market price of `sETH` is returning `105`
    - When a user attempts to exchange `sBTC` (or any other synth) into `sETH`
      - Then as `105` is less than `0.66x` away from `200` (i.e. it's above `100`), then the exchange will continue, and `105` will be persisted as the last price.
        ...
        - And the `user` attempts to `exchange` `sETH` into any other synth
          - Then as the price of `5` (the price at three minutes after their exchange) is more than `0.66x` away from `105` (i.e. below `52.5`), then the settlement will process with no reclaim or rebate.
