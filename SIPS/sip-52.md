...
3. Add an internal function that will emit an event `mappingUpdated` whenever a `mapping()` is changed.
4. `ExchangeRates` needs new function to `getNextRoundId(fromRoundId, bytes32)`
5. `Synthetix.isWaitingPeriod(bytes32)` would need to be amended, if any exchange into (`dest`), out of (`src`) (or both) a "next price" synth, then needs to lookup in `ExchangeRates.getNextRoundId()` to see if a new price has been received or not (for one or both)
6. `Exchanger.settle()` would need to determine next price for `src` and `dest` using either timed waiting period (as now) or "next price" (via #3 above), depending on each synth's parameters from #1 above.
...
## Event
```solidity
event mappingUpdated(bytes32 src, bool mapping);
```
