...
interface ITradingIncentive {
  // Views
  function rewards(uint periodID) external view; 
  
  function rewardForPeriods(uint[] periodIDs) external view;
...
A function to allow trading incentives to be recorded against any `address` instead of the `msg.sender` of the `synthetix.exchange()` event. The `address` will need to be the one claiming the rewards from the `trading incentive` contract.    
