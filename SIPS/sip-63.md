...
Referrals - A function to allow trading incentives to be recorded against any `address` instead of the `msg.sender` of the `synthetix.exchange()` transaction. The `address` will need to be the one claiming the rewards from the `trading incentive` contract.
...
In order to support and integrate partner exchange volume tracking, the `Synthetix.exchange` function will be overrided to allow partner exchanges to pass in their unique tracking code (for the partner volume tracking) and also the `referrer` address that will be able to track the originating trader's individual address.
...
interface ISynthetix {
  ...
  // overloading exchangeOnBehalf function
